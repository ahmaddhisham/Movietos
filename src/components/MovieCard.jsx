...
function MovieCard({ movie }) {
  ...
  const handleFavoriteClick = useCallback(
    ...
    [isMovieFavorite, movie, addToFavorites, removeFromFavorites, isProcessing, showTemporaryFeedback]
  );
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-1">
          <p className="text-sm text-gray-400" aria-label={`Released in ${releaseYear}`}>
            {releaseYear}
          </p>
      ...
      >
        ...
        <span 
          ...
          className={`block transition-all duration-300 ${
            ...
            isMovieFavorite ? "scale-125" : "scale-100"
          }`}
        >
          {isProcessing ? (
            <span className="inline-block h-4 w-4 border-2 border-white border-t-transparent rounded-full" />
          ) : (
            <span className={`transition-colors ${isMovieFavorite ? "text-white" : "text-gray-300 hover:text-white"}`}>
              {isMovieFavorite ? "❤️" : "🤍"}
            </span>
          )}
        </span>
        ...
        <div className="absolute left-1/2 top-4 z-10 -translate-x-1/2 animate-fade-in-up">
          <div className="rounded-lg bg-gray-900/95 px-4 py-2 text-sm text-white shadow-xl backdrop-blur-sm">
            <div className="flex items-center gap-2">
              <span className={isMovieFavorite ? "text-red-500" : "text-green-500"}>
                {isMovieFavorite ? "✓" : "❤️"}
              </span>
